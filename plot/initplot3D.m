...
Y_M3D = 0           + Rm2*Y_M3D;
Z_M3D = 0           + Rm2*Z_M3D;
surf(X_M3D*Lf, Y_M3D*Lf, Z_M3D*Lf, 'FaceColor', 'g', 'FaceLighting', 'none', 'EdgeColor', 'none');
...
%     [texturemap,~]=imread('Earth.jpg');
%     surf(gca, X_E3D*Lf, Y_E3D*Lf, Z_E3D*Lf,texturemap, 'LineStyle', 'none','FaceColor','texturemap');
...
if(params.plot.allLibPoints)
    ...
    Li = cr3bp.l1.position;
    plot3(Li(1)*Lf, Li(2)*Lf, Li(3)*Lf, '*', 'Color',  rgb('dark red'),  'MarkerFaceColor',  rgb('dark red'));
    if(params.plot.names)
        text(Li(1)*cr3bp.L, 0, 0.1301*Lf, 'L_1');
    end
...
function [] = sun_mars_system(cr3bp, params)
...
Y_M3D = 0           + Rm2*Y_M3D;
Z_M3D = 0           + Rm2*Z_M3D;
surf(X_M3D*Lf, Y_M3D*Lf, Z_M3D*Lf, 'FaceColor', 'r', 'FaceLighting', 'none', 'EdgeColor', 'none');
% surf(X_M3D*Lf, Y_M3D*Lf, Z_M3D*Lf, 'FaceColor', [0 0.6  0.9], 'FaceLighting', 'none', 'EdgeColor', 'none');
